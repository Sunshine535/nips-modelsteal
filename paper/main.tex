...
\begin{itemize}
    ...
    \item In a controlled case study on Qwen2.5-0.5B (494M parameters, 24 layers), we empirically show that the $k \times k$ sketched Gramian has full rank ($= k$, certifying $\geq k$ observable directions) for all tested configurations ($K \in \{8, 32, 64, 128\}$, $k \in \{64, 128, 256\}$), with eigenvalues scaling linearly in $K$ and condition number $\kappa \approx 1.9$--$3.6$.  Yet despite well-conditioned sketched Gramians, no initialization strategy recovers any weight matrix ($\cos \approx 0$ for all projections; block mean $\approx 0.12$--$0.14$ driven by LayerNorm).  This decoupling between \emph{partial sampled observability} and \emph{recoverability} is our central empirical finding (\S\ref{sec:experiments}).
...
\begin{enumerate}
    ...
    \item \textbf{Attention V/O and Q/K scaling} $(\R_{>0})^{2H_{\text{kv}}}$: In GQA with $H_{\text{kv}}$ shared KV groups, the value projection $\bW_V^{(g)}$ is shared across all $H/H_{\text{kv}}$ query heads in group $g$.  Scaling $\bW_V^{(g)} \mapsto \beta_g \bW_V^{(g)}$ (and its bias $\mathbf{b}_V^{(g)} \mapsto \beta_g \mathbf{b}_V^{(g)}$) together with $\bW_O^{(h)} \mapsto \beta_g^{-1} \bW_O^{(h)}$ for all heads $h$ in group $g$ preserves the output, contributing $H_{\text{kv}}$ continuous degrees of freedom.  A similar Q/K scaling symmetry (scaling $\bW_K^{(g)} \mapsto \alpha_g^{-1} \bW_K^{(g)}$ and all Q heads in the group $\bW_Q^{(h)} \mapsto \alpha_g \bW_Q^{(h)}$, including biases) contributes $H_{\text{kv}}$ additional directions, for a total of $2H_{\text{kv}}$ attention gauge directions per block.
\end{enumerate}
\end{definition}

For Qwen2.5-0.5B with $d = 896$, $d_\text{ff} = 4864$, $H=14$ heads, and $H_{\text{kv}}=2$ KV heads, each block has $2d + d_\text{ff} + 2H_{\text{kv}} = 6660$ gauge directions. Discrete symmetries (attention head permutations $\mathfrak{S}_H$ and FFN neuron permutations $\mathfrak{S}_{d_\text{ff}}$) also exist but are handled post-hoc via Hungarian matching.

\begin{remark}
The gauge directions form the Lie algebra $\mathfrak{g}$ of $\cG$. We construct an orthonormal basis $\bU \in \R^{p \times d_g}$ for the tangent space at the current parameterization, where $d_g = 2d + d_\text{ff} + 2H_{\text{kv}}$ per block in the suffix.  Note that our current code implementation includes only the RMSNorm and MLP gauge; attention scaling symmetries are omitted from the projection, which means the computed Gramian may have additional (small) null-space directions not accounted for.  Since $2H_{\text{kv}} \ll d_\text{ff}$, this omission does not materially affect our results but should be addressed in future work.
\end{remark}
...
$\bG(\cQ)$ is a symmetric positive semi-definite matrix. Its null space contains parameter directions that produce \emph{zero logit change} across all queries in $\cQ$ at all observed positions --- these directions are fundamentally unidentifiable.
...
\end{equation}
where $\bH_\ell$ is flattened to $\R^{Td}$.  The $Td$-dimensional hidden state is the bottleneck:
\begin{equation}\label{eq:rank-bound}
    \rank\bigl(\bG_\perp^{(\ell)}(\cQ)\bigr) \leq \min\!\Bigl(p_\ell - d_{g,\ell},\;\; Td \cdot |\cQ|\Bigr).
\end{equation}
For Qwen2.5-0.5B with $d=896$, $T=128$, and $p_\ell = 14.9\text{M}$, even a single query gives rank $\leq Td = 114{,}688$; with $N = 2048$ queries, $\rank \leq N \cdot Td \approx 235\text{M} \gg p_\ell$, so the binding constraint becomes $p_\ell - d_{g,\ell}$.
\end{theorem}
\begin{proof}
The Gramian $\bG^{(\ell)} = \frac{1}{N}\sum_i \bJ_{\bx_i,\ell}^\top \bJ_{\bx_i,\ell}$ has $\rank(\bG^{(\ell)}) \leq \rank([\bJ_{\bx_1,\ell}; \ldots; \bJ_{\bx_N,\ell}])$.  Each $\bJ_{\bx_i,\ell}$ factors through $\bH_\ell \in \R^{Td}$ via~\eqref{eq:rank-bound-factor}.  Since $\rank(AB) \leq \min(\rank(A), \rank(B))$, we have $\rank(\bJ_{\bx_i,\ell}) \leq Td$ per query.  Stacking $N$ queries: $\rank \leq N \cdot Td$.  Gauge projection reduces the effective rank by at most $d_{g,\ell}$.  Note that the cross-position coupling from self-attention means rank per query can be as large as $Td$, not merely $d$ per observed position.
\end{proof}
...
\begin{equation}\label{eq:screening}
    \bG^{(\ell)}(\cQ) = \frac{1}{N}\sum_{i=1}^{N} \cB_i^\top \cdot \cA_i^\top \bW_{\mathrm{lm,ext}}^\top \bW_{\mathrm{lm,ext}} \cA_i \cdot \cB_i,
\end{equation}
where $\cA_i = \partial F_{\ell+1:L}/\partial \bH_\ell|_{\bx_i} \in \R^{Td \times Td}$ is the full sequence-level downstream Jacobian (including cross-position coupling from self-attention), $\cB_i = \partial \bH_\ell / \partial \btheta_\ell|_{\bx_i} \in \R^{Td \times p_\ell}$ is block $\ell$'s parameter Jacobian, and $\bW_{\mathrm{lm,ext}}$ extracts the last $K_\text{pos}$ positions and applies $\bW_{\text{lm}}$.  Both $\cA_i$ and $\cB_i$ are query-dependent and cannot be factored out of the sum.  The projected Gramian is $\bG_\perp^{(\ell)} = P_\perp \bG^{(\ell)} P_\perp$.

The maximum singular value of each summand is bounded by $\sigma_{\max}(\bW_{\mathrm{lm,ext}} \cA_i \cB_i)^2 \leq \sigma_{\max}(\bW_{\mathrm{lm,ext}})^2 \cdot \sigma_{\max}(\cA_i)^2 \cdot \sigma_{\max}(\cB_i)^2$.  The downstream Jacobian $\cA_i$ is a composition of block-level sequence Jacobians.  When these are contractive ($\sigma_{\max} < 1$), $\sigma_{\max}(\cA_i)$ decays exponentially with $L - \ell$, suppressing each summand.  Conversely, residual connections typically ensure $\sigma_{\max} \geq 1$, so deeper blocks need not have weaker signal in practice.  This provides a qualitative explanation for observed depth-dependent Gramian spectra, though the actual screening depends on the specific Jacobians at the current parameterization.
\end{theorem}
...
\begin{table}[t]
\centering
\caption{\textbf{Expanded-observation training} (alg\_clean init, seed~42).  Despite a full-rank sketched Gramian ($\kappa{=}2.4$, certifying $\geq 128$ observable directions), only RMSNorm parameters recover; all weight matrices remain at $\cos \approx 0$.  Increasing $K$ from 32 to 128 does not improve recovery, confirming that the bottleneck is optimization, not observation.}
\label{tab:expanded}
\small
...
\begin{table}[t]
\centering
\caption{\textbf{Warm-start initialization sweep} (Block~23 only, $K{=}32$, oracle regime with boundary injection, seed~42).  Student \emph{block-level} parameters are interpolated: $\alpha \cdot \btheta^*_{23} + (1{-}\alpha) \cdot \btheta_{\text{rand}}$.  Training does not improve cosine beyond the initial value ($|\Delta| \leq 0.01$); recovery is determined entirely by initialization distance.  This is evidence for a local optimization barrier on this block under oracle injection, not a full-model basin analysis.}
\label{tab:warmstart}
\small
\begin{tabular}{@{}c ccc@{}}
\toprule
...
\begin{itemize}
    ...
    \item \textbf{Observation window.} Our v3 diagnostic tests $K \leq 128$ positions and $k \leq 256$ probes, always finding full sketched rank (equal to $k$).  The true Gramian rank is likely much larger (bounded by $d \cdot N = 896 \times 2048 \approx 1.8\text{M}$ by Theorem~\ref{thm:rank-bound}), but computing it precisely would require $k$ of that order, each involving a JVP evaluation---computationally prohibitive.  Our claim that ``observability is not the only bottleneck'' is supported by the full-rank sketch at $k{=}256$ and the failure to improve recovery at $K{=}32$, but does not characterize the complete eigenspectrum.
...
In a controlled case study on Qwen2.5-0.5B, we find a nuanced \textbf{negative parameter identifiability result}.  Our diagnostic experiments reveal that the $k \times k$ sketched Gramian has full rank (equal to $k$, the number of probes) for all tested observation windows: expanding from $K{=}8$ to $K{=}128$ suffix positions yields well-conditioned spectra ($\kappa \approx 2.4$) with eigenvalues scaling linearly in $K$.  This certifies that at least $k = 128$ parameter directions are observable, but does not establish full local identifiability of all $14.9\text{M}$ parameters.  Yet despite this partial observability evidence, no initialization strategy recovers transformer weight matrices ($\cos \approx 0.12$--$0.14$ across all configurations, consistent with initialization bias).  A block-local warm-start experiment (Block~23, oracle injection) is consistent with an optimization-distance barrier: interpolating just 10\% of the teacher weights into the initialization raises cosine from $0.13$ to $0.42$, yet gradient-based training contributes $|\Delta\cos| < 0.01$ at any interpolation level.  These findings provide evidence for an \emph{observability--recoverability gap}: even partial observability (at least $k$ directions confirmed observable) does not translate into practical parameter recovery via gradient-based optimization in high-dimensional, non-convex landscapes, at least for the architecture and scale studied here.
...
In GQA, the value projection $\bW_V^{(g)}$ is shared across all $H/H_{\text{kv}}$ query heads in KV group $g$.  Scaling $\bW_V^{(g)} \mapsto \beta_g \bW_V^{(g)}$ (and bias $\mathbf{b}_V^{(g)} \mapsto \beta_g \mathbf{b}_V^{(g)}$) together with $\bW_O^{(h)} \mapsto \beta_g^{-1} \bW_O^{(h)}$ for all heads $h$ in group $g$ preserves the output.  The tangent vector for KV group $g$ is:
\begin{equation}
    \mathbf{t}_g^{\text{V/O}} = (\underbrace{+\bW_V^{(g)}, +\mathbf{b}_V^{(g)}}_{\text{on V group } g}, \underbrace{-\bW_O^{(h)} \text{ for } h \in \text{group } g}_{\text{on O heads in group } g}).
\end{equation}
This contributes $H_{\text{kv}}$ gauge directions.  Similarly, Q/K scaling ($\bW_K^{(g)} \mapsto \alpha_g^{-1}\bW_K^{(g)}$, $\bW_Q^{(h)} \mapsto \alpha_g \bW_Q^{(h)}$ for $h$ in group $g$, including biases) adds $H_{\text{kv}}$ additional directions, for a total of $2H_{\text{kv}}$ attention gauge directions per block.

\subsection{Gauge Dimensions}

For Qwen2.5-0.5B ($d=896$, $d_\text{ff}=4864$, $H=14$ attention heads, $H_{\text{kv}}=2$ KV heads, 24 blocks):
\begin{itemize}
    \item Per block: $2 \times 896 + 4864 + 2 \times 2 = 6660$ gauge directions.
    \item Final norm: $+896$ directions.
    \item For $K=2$ suffix blocks: $2 \times 6660 + 896 = 14{,}216$ total gauge directions.
    ...
    \item \emph{Note}: Our current code implementation omits the $2H_{\text{kv}} = 4$ attention scaling directions per block from the gauge projection.  Since these contribute only $0.03\%$ of the total gauge directions, this omission has negligible practical impact on the projected Gramian spectra.
\end{itemize}
